...
namespace hanabi
{
    public class HanabiInteractive
    {
        ...
        private string PrepareDebugInfo()
        {
            ...
            var firstPlayer = $"{firstPlayerLabel + ": ",labelWidth}" + join(FirstPlayer);
            var secondPlayer = $"{secondPlayerLabel + ": ",labelWidth}" + join(SecondPlayer);
        ...
        {
            foreach (var color in card.GetAvailableColors())
                foreach (var rank in card.GetAvailableRanks())
                    if (IsRisky(color, rank))
                        return true;
